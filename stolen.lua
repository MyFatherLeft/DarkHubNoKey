...
local lib = loadstring(game:HttpGet("https://raw.githubusercontent.com/MyFatherLeft/DarkHubNoKey/main/lib.lua",true))()
...
function getclosest()
    local closest, pos, maxdist = nil, nil, Client.Toggles.UseFov and FovCircle.Radius or math.huge
...
local upvector = Vector3.new(0, 4.5, 0)
local currenttime = tick()
local lastpos = nil
local tpcount = 0
function setpos(pos)
    local newtimee = tick()
    if currenttime > newtimee then
        currenttime = currenttime + 0.000001
        newtimee = currenttime
    end

    lastpos = pos
    send(network, "repupdate", pos, Vector2.new(camera.angles.X, camera.angles.Y), newtimee)
end

function move(origin, target)
    local cframe = CFrame.new(origin, target)

    for i = 1, math.floor((origin - target).Magnitude / 9.5) do
        setpos(cframe * Vector3.new(0, 0, -9.5 * i))
    end
...
debug.setupvalue(loadgun, 59, function(...)
